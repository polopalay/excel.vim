...
def get_sheet(wb, sheet_name=None) -> Worksheet:
    """Lấy đúng worksheet theo tên, hoặc sheet đầu tiên nếu không chỉ định."""
    if sheet_name:
        try:
...
def get_merge_ranges(ws):
    ...
    Dùng cách "chuẩn" qua openpyxl (ws.merged_cells.ranges).
...
# ----------------------------------------------------------------------------
# Helper render / parse bảng ASCII
# ----------------------------------------------------------------------------
...
# ============================================================================
# CORE FUNCTIONS - OPEN / SAVE XLSX
# ============================================================================


def open_xlsx(path, sheet_name=None):
    """Đọc file .xlsx và in ra bảng ASCII tương ứng (dùng cho lệnh CLI 'open')."""
...
# ============================================================================
# CORE FUNCTIONS - QUẢN LÝ SHEET
# ============================================================================


def list_sheets(path):
    """In ra danh sách tên tất cả các sheet trong file .xlsx."""
...
def create_sheet(path, sheet_name):
    """Tạo 1 sheet mới với tên chỉ định."""
    ensure_workbook(path)
...
def rename_sheet(path, old_name, new_name):
    """Đổi tên 1 sheet từ old_name -> new_name."""
    ensure_workbook(path)

    wb = load_workbook(path)

    if old_name not in wb.sheetnames:
        raise RuntimeError(f"Sheet not found: {old_name}")

    if new_name in wb.sheetnames:
        raise RuntimeError(f"Sheet already exists: {new_name}")
...
def delete_sheet(path, sheet_name):
    """Xoá 1 sheet (không cho xoá nếu đó là sheet cuối cùng còn lại)."""
    ensure_workbook(path)
    ...
    if len(wb.sheetnames) == 1:
        raise RuntimeError("Cannot delete last sheet")
...
# ============================================================================
#
# Cách dùng (do excelPlugin.vim gọi qua systemlist()):
#   python excel.py open <xlsx_file> [sheet_name]
#       -> mở file .xlsx và hiển thị nội dung sheet dưới dạng bảng ASCII
#   python excel.py save <xlsx_file> <txt_file> [sheet_name]
#       -> lưu nội dung bảng ASCII (txt_file) trở lại file .xlsx,
#          giữ nguyên cấu trúc merge ban đầu
#   python excel.py sheets <xlsx_file>
#       -> liệt kê tên tất cả các sheet trong file .xlsx
#   python excel.py addsheet <xlsx_file> <sheet_name>
#       -> tạo sheet mới
#   python excel.py rensheet <xlsx_file> <old_name> <new_name>
#       -> đổi tên sheet
#   python excel.py delsheet <xlsx_file> <sheet_name>
#       -> xoá sheet
# ----------------------------------------------------------------------------
